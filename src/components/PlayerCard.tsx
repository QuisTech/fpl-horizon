  ...
  return (
    <div className={cn(
      ...
      <div className="absolute opacity-0 group-hover:opacity-100 transition-opacity z-50 bg-slate-900/95 backdrop-blur-sm border border-slate-700 text-slate-300 text-[9px] p-2 rounded shadow-2xl w-32 bottom-full mb-2 left-1/2 -translate-x-1/2 pointer-events-none">
        ...
        <div className="flex justify-between"><span>Raw xP:</span> <span className="text-fpl-green font-mono">{typeof player.xP === 'number' ? player.xP.toFixed(2) : '—'}</span></div>
        <div className="flex justify-between"><span>Cost:</span> <span className="font-mono">£{(player.now_cost/10).toFixed(1)}M</span></div>
        <div className="flex justify-between font-bold border-t border-slate-800 pt-1 mt-1"><span>ROI:</span> <span className="text-cyan-400 font-mono">{typeof player.xP === 'number' ? (player.xP / (player.now_cost / 10)).toFixed(2) : '—'}</span></div>
      </div>
    </div>
  );
};
