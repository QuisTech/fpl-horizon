...
export const useFPLData = (riskMode: 'safe' | 'aggressive' | 'value') => {
  ...
  const [history, setHistory] = useState<any>(() => {
    const saved = localStorage.getItem('fpl_horizon_history') || localStorage.getItem('fpl_strategist_history') || localStorage.getItem('fpl_optimizer_history');
    return saved ? JSON.parse(saved) : {};
  });

  useEffect(() => {
    localStorage.setItem('fpl_horizon_history', JSON.stringify(history));
  }, [history]);
      ...
      .then(res => {
        if (res.data?.history && typeof res.data.history === 'object' && Object.keys(res.data.history).length > 0) {
          setHistory((prev: any) => {
            ...
            localStorage.setItem('fpl_horizon_history', JSON.stringify(merged));
            return merged;
          });
        }
      })
  ...
  const takeSnapshot = async (gwId: number, currentModeData: RecommendationResponse, mode: string) => {
    ...
    localStorage.setItem('fpl_horizon_history', JSON.stringify(newHistory));
  ...
  const clearConstraints = () => {
    ...
    try {
      ...
      localStorage.setItem('fpl_horizon_history', JSON.stringify(currentHistory));
